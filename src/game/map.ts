...
class MapController {
    ...
    public update(ship: Ship, deltaTime: number): void {
        ...
        for (const layer of this.starLayers) {
            ...
            for (const star of layer.stars) {
                if (star.x < regionLeft || star.x > regionRight || star.y < regionTop || star.y > regionBottom) {
                    ...
                    star.color = Math.random() > 0.91 ? nuttyStarColor() : `rgba(255, 255, 255, ${Math.max(0.3, Math.random())})`;
                }
            }
        }
    }
    ...
    public handleResize(width: number, height: number): void {
        ...
        const starsPerLayer = Math.round(this.canvasWidth * this.canvasHeight * starDensity);
        for (const layer of this.starLayers) {
    ...
    public render(context: CanvasRenderingContext2D, ship: Ship): void {
        ...
        for (const layer of this.starLayers) {
            ...
            for (const star of layer.stars) {
                ...
                starX = ((starX % this.canvasWidth) + this.canvasWidth) % this.canvasWidth;
                starY = ((starY % this.canvasHeight) + this.canvasHeight) % this.canvasHeight;
