...
import fs, { readFileSync, WriteStream } from "node:fs";
 ...
 */
function generateRandomShape(vertexCount: number, baseRadius: number, variance: number): Vertex[] {
    const shape: Vertex[] = [];
 ...
 */
export function generateSpaceObjects(count: number, worldWidth: number, worldHeight: number): SpaceObject[] {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const fillStyle = possibleColors[Math.floor(Math.random() * possibleColors.length)];
...
const spaceObjectsToRegions = (spaceObjects: SpaceObject[], regionSize: number): Map<string, SpaceObject[]> => {
...
export const getRegionKeys = (regions: Map<string, SpaceObject[]>): string[] => {
    return Array.from(regions.keys());
}

export const getRegionFromCoords = (regions: Map<string, SpaceObject[]>, regionSize: number, x: number, y: number): SpaceObject[] => {
...
export const getSpaceObjectsInArea = (left: number, right: number, top: number, bottom: number): SpaceObject[] => {
    console.log("Getting objects in area", left, right, top, bottom);
    const spaceObjectJson = readFileSync(join(import.meta.dirname, "objects.json"), "utf-8");
...
export const loadRegionFromCoords = (x: number, y: number): SpaceObject[] => {
    const regions = readFileSync(join(import.meta.dirname, "regions.json"), "utf-8");
