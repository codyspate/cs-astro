...
export class Bullet {
    ...
    constructor(
        ...
        radius: number,
        maxRange: number,
        shipVelocityX: number = 0,
        shipVelocityY: number = 0
    ) {
    ...
    public render(context: CanvasRenderingContext2D, screenX: number, screenY: number): void {
        context.beginPath();
...
export class Weapon {
    ...
    public ammo: number;
    public maxAmmo: number = 150;

    private mouseMode: boolean = false;

    constructor(fireRate: number, bulletSpeed: number, bulletRadius: number, maxBulletRange: number) {
    ...
    public update(ship: Ship, deltaTime: number, input: InputController): void {
        ...
        const firingAngle = this.mouseMode ? Math.atan2(input.mouseY - ship.posY, input.mouseX - ship.posX) : ship.angle;
        ...
        if (this.isFiring) {
            ...
            if (currentTime - this.lastFireTime > this.fireRate) {
                ...
                const angleDiff = Math.abs(Math.atan2(Math.sin(firingAngle - ship.angle), Math.cos(firingAngle - ship.angle)));
